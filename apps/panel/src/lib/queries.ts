...
export type ActaListItem = {
  ...
  votos_nulos: number;
  submitted_at: string;
  mesas: { numero_mesa: number; recintos: { nombre: string } } | null;
...
export async function obtenerActas(): Promise<ActaListItem[]> {
  const { data, error } = await supabase
    .from("actas")
    .select(
      "id, estado, votos_blancos, votos_nulos, submitted_at, mesas ( numero_mesa, recintos ( nombre ) ), contests ( nombre )"
    )
